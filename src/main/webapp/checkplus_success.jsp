...
<%
    ...
    String sSiteCode = "AB492";				   // NICE로부터 부여받은 사이트 코드
    String sSitePassword = "RkyG69i03dFo";			 // NICE로부터 부여받은 사이트 패스워드
...
<script type="text/javascript">
	...
	try {
		$("#mberNm", opener.document).val("<%= sName %>");
		$("#birth_day", opener.document).val("<%= sBirthDate %>");
		$("#ci", opener.document).val("<%= sConnInfo %>");
		$(opener.document).find("#phone_form").submit();
        window.close();
	} catch (err){
		alert("처리 중 오류["+err+"]");
	}
