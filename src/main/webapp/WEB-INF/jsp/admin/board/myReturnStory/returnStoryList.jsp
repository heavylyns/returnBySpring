		    ...
		    <table class="tbl">
		        ...
		        <tbody>
	 				...
	 				<c:if test="${fn:length(resultList) == 0}">
	                    <tr>
	                        <td class="tc" colspan="5">등록된 나의 귀농귀촌 이야기가 없습니다.</td>
	                    </tr>
	 				</c:if>
		        </tbody>
		    </table>
		    <div class="clearfix">
		    	<%-- <%if(s_bbsId2.equals("BBSMSTR_000000000001")){ %>
					<c:if test="${fn:contains(curAdmin.menuIds, '[312]')}">
		            	<a href="javascript:fn_egov_addNotice();" class="message form-line-btn defbtn pink fr">글쓰기</a>
		            </c:if>
		        ...
		        <%}else if(s_bbsId2.equals("BBSMSTR_000000000003")){ %>
					<c:if test="${fn:contains(curAdmin.menuIds, '[332]')}">
		           		<a href="javascript:fn_egov_addNotice();" class="message form-line-btn defbtn pink fr">글쓰기</a>
		           	</c:if>
		        <%}else if(s_bbsId2.equals("BBSMSTR_000000000004")){ %>
					<c:if test="${fn:contains(curAdmin.menuIds, '[342]')}">
		            	<a href="javascript:fn_egov_addNotice();" class="message form-line-btn defbtn pink fr">글쓰기</a>
		            </c:if>
		        <%}else if(s_bbsId2.equals("BBSMSTR_000000000005")){ %> 
					<c:if test="${fn:contains(curAdmin.menuIds, '[352]')}">--%>
					<c:if test="${fn:contains(curAdmin.menuIds, '[342]')}">
