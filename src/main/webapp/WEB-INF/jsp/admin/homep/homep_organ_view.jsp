...
%>
		...
		<script type="text/javascript">
			...
			function organ_agree(v_type){
				...
				if(v_type == "insert"){
					document.frm.action = "<c:url value='/admin/homep/organ_join.do'/>";
				}else{
					document.frm.action = "<c:url value='/admin/homep/organ_update.do'/>";
				}
				document.frm.submit();
			}

			function organ_delete(){
				if(confirm("정말로 삭제하시겠습니까?")){
					document.frm.action = "<c:url value='/admin/homep/organ_delete.do'/>";
					document.frm.submit();
				}
			}

			function organ_file_delete(){
				if(confirm("정말로 첨부파일을 삭제하시겠습니까?")){
					document.frm.action = "<c:url value='/admin/homep/organ_file_delete.do'/>";
					document.frm.submit();
				}
			}
		...
		<div class="container" id="container">
			...
			<div class="contents">
	            ...
	            <form action="#" name="frm" id="frm" method="post" enctype="multipart/form-data">
	                ...
	                <fieldset>
	                    <legend class="screen_out">관련기관 관리</legend>
	                    <table class="tbl no-thead write">
	                        <caption>관련기관 표</caption>
	                        <colgroup>
	                            <col style="width:25%;"/>
	                            <col style="width:55%;"/>
	                            <col style="width:20%;"/>
	                        ...
	                        <tbody>
	                        <tr>
	                            <th><label for="title">기관명</label></th>
	                        ...
	                        <tr>
	                            ...
	                            <td>
	                            	<input type="file" id="organ_file" name="organ_file" class="wid100"/>
	                            ...
	                            <td class="table-btn"><button type="button" class="defbtn pink valdel-btn" onclick="javascript:organ_file_delete();">첨부파일 삭제</button></td>
	                        </tr>
	                        <tr>
	                    ...
	                    <div class="defbtn-wrap">
	                    	<%if(s_type.equals("update")){ %>
		                        <button type="button" class="defbtn pink" onclick="javascript:organ_agree('update')">수정</button>
		                        <button type="button" class="defbtn white" onclick="javascript:organ_delete();">삭제</button>
	                        <%}else{ %>
		                        <button type="button" class="defbtn pink" onclick="javascript:organ_agree('insert')">등록</button>
