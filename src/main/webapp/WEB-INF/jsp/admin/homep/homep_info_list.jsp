...
<%@ taglib  prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
		...
		<script type="text/javascript">
			...
			function infoListGo(pageNo) {
				document.frm.pageIndex.value = pageNo;
				//$("#pageIndex").val(pageNo);
				document.frm.action = "<c:url value='/admin/homep/homep_info_list.do'/>";
				document.frm.submit();
			}
		</script>
		...
		<div class="container" id="container">
	        ...
	        <div class="breadcrumb">
			    <ul class="">
			        ...
			        <li class="active">인포존 관리</li>
			    </ul>
			</div>
			
			<h3 class="subcont-tit">인포존 관리</h3>
	            ...
	            <table class="tbl image">
	                ...
	                <tbody class="tc">
						<c:forEach var="result" items="${resultList}" varStatus="status">
		                    <tr>
		                        <td>${result.rownum }</td>
		                        <td><a href="/admin/homep/homep_info_view.do?popup_no=${result.popup_no }&type=update">${result.title }</a></td>
		                        <td>
		 				...
		 				<c:if test="${fn:length(resultList) == 0}">
		                    <tr>
		                        <td colspan="7">등록된 관련기관이 없습니다.</td>
		                    </tr>
		 				</c:if>
	                </tbody>
	            </table>
	            <div class="clearfix">
	                <a href="/admin/homep/homep_info_view.do?type=insert" class="message form-line-btn defbtn pink fr">등록</a>
	            </div>
		    ...
		    <form name="frm" action ="<c:url value='/admin/homep/homep_info_list.do'/>" method="post" enctype="multipart/form-data">
				<input type="hidden" name="pageIndex" value="<c:out value='${boardVO.pageIndex}'/>"/> 
			        ...
			        <div class="tbl-search">
			            ...
			            <button type="button" class="tbl-search-btn" onclick="javascript:infoListGo('1');"><span class="screen_out">검색</span></button>
			        </div>
		        </form>
